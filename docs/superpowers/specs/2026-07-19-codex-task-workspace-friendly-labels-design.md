...
为了让 core 层保持与语言无关，`ProjectSummary` 增加结构化的 `labelKind` 和 `labelSuffix`。`label` 保存基础名称，`labelSuffix` 保存可选的环境、日期、时间或 basename 消歧后缀。渲染器在 `labelKind: "codex-task-untitled"` 时根据当前语言生成“未命名会话”或“Untitled session”，其他情况直接使用 `label`，最后统一拼接 `labelSuffix`。普通路径标签和有标题的任务标签分别使用 `labelKind: "path"` 与 `labelKind: "codex-task-title"`。
...
消歧按以下顺序执行：先保留“同一物理路径出现在多个环境”时的环境名称后缀，再在每个环境内部对任务标题执行日期、时间和 basename 消歧。环境后缀不参与同一环境内的标题重复计数。
