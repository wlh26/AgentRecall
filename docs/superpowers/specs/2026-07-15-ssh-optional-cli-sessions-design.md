...
- 不改变 Optional sources 的默认关闭策略；远程同步只扫描已开启的扩展来源，关闭后同时清理已经索引的同来源会话。
...
主进程每次同步前读取当前 Optional sources 设置，把已开启来源传给 collector。基础 Claude/Codex 始终扫描；TClaude、TCodex、CodeBuddy 只有开关打开时才进入候选列表。设置关闭后复用现有 `pruneDisabledOptionalSources` 清理本地和 SSH 环境中的对应来源。
